...
from data.dataset_dali import Dataset_imagenet_dali
...
from model.student.DenseNet_sparse import DenseNet_40_sparse_cifar10
...
class TrainDDP:
    ...
    def dataload(self):
        if self.dali and self.dataset_type == "imagenet":
            dataset = Dataset_imagenet_dali(
                self.dataset_dir,
    ...
    def train(self):
        ...
        if self.arch == "GoogLeNet":
            ...
            self.student = DDP(self.student, find_unused_parameters=False)
        ...
        for epoch in range(self.start_epoch + 1, self.num_epochs + 1):
            ...
            with tqdm(total=len(self.train_loader), ncols=100) as _tqdm:
                ...
                for data in self.train_loader:
                    ...
                    reduced_prec5 = self.reduce_tensor(prec5)

                    if self.rank == 0:
                        n = images.size(0)
                        meter_oriloss.update(reduced_ori_loss.item(), n)
                        meter_kdloss.update(
                            self.coef_kdloss * reduced_kd_loss.item(), n
                        )
                        meter_rcloss.update(
                            self.coef_rcloss
                            * reduced_rc_loss.item()
                            / len(feature_list_student),
                            n,
                        )
                        meter_maskloss.update(
                            self.coef_maskloss * reduced_mask_loss.item(),
                            n,
                        )

                        meter_loss.update(reduced_total_loss.item(), n)
                        meter_top1.update(reduced_prec1.item(), n)
                        meter_top5.update(reduced_prec5.item(), n)
            ...
            if self.rank == 0:
                ...
                self.logger.info(
                    "[Train mask avg] Epoch {0} : ".format(epoch) + str(masks)
                )
